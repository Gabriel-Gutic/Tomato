...
#define Vec2Node(Type) template<>												\
struct convert<Tomato::Type##2> {														\
	static Node encode(const Tomato::Type##2& v2) {										\
		Node node;																\
		node.push_back(v2.x);													\
		node.push_back(v2.y);													\
		return node;															\
																				...
																				\
	static bool decode(const Node& node, Tomato::Type##2& v2) {							\
		if (!node.IsSequence() || node.size() != 2) {							\
			return false;														\
		}																		\
																				\
		v2.x = node[0].as<Tomato::Type>();												\
		v2.y = node[1].as<Tomato::Type>();												\
		return true;															\
	}																			\
};


#define Vec3Node(Type) template<>												\
struct convert<Tomato::Type##3> {														\
	static Node encode(const Tomato::Type##3& v3) {										\
		Node node;																\
		node.push_back(v3.x);													\
		node.push_back(v3.y);													\
		node.push_back(v3.z);													\
		return node;															\
																				...
																				\
	static bool decode(const Node& node, Tomato::Type##3& v3) {							\
		if (!node.IsSequence() || node.size() != 3) {							\
			return false;														\
		}																		\
																				\
		v3.x = node[0].as<Tomato::Type>();												\
		v3.y = node[1].as<Tomato::Type>();												\
		v3.z = node[2].as<Tomato::Type>();												\
		return true;															\
	}																			\
};


#define Vec4Node(Type) template<>												\
struct convert<Tomato::Type##4> {														\
	static Node encode(const Tomato::Type##4& v4) {										\
		Node node;																\
		node.push_back(v4.x);													\
		node.push_back(v4.y);													\
		node.push_back(v4.z);													\
		...
		return node;															\
																				...
																				\
	static bool decode(const Node& node, Tomato::Type##4& v4) {							\
		if (!node.IsSequence() || node.size() != 4) {							\
			return false;														\
		}																		\
																				\
		v4.x = node[0].as<Tomato::Type>();												\
		v4.y = node[1].as<Tomato::Type>();												\
		v4.z = node[2].as<Tomato::Type>();												\
		v4.w = node[3].as<Tomato::Type>();												\
		return true;															\
	}																			\
};