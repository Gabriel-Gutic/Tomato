...
{
	Tomato::App::GetScenes()["Tilemap Scene"]->GetCamera()->GetComponent<Tomato::Transform>().Position = Tomato::Float3(0.0f, 0.0f, 12.0f);
...
void TilemapLayer::SetTilePosition(Tile& tile, Tomato::Int i, Tomato::Int j)
{
	...
	tile.Quad.GetComponent<Tomato::Transform>().Position = pos;
}

Tile::Tile(Tomato::UInt row, Tomato::UInt col, Tomato::UInt rowspan, Tomato::UInt colspan)
	:Row(row), Col(col), Rowspan(rowspan), Colspan(colspan)
{
	Quad.AddComponent<Tomato::Transform>().Scale = Tomato::Float3((Tomato::Float)Rowspan, (Tomato::Float)Colspan, 1.0f);
}
