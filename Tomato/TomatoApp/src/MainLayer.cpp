	...
	:m_Scene(Tomato::App::GetScenes()["Main Scene"])
{
	...
	else m_PolygonNOS = m_Scene->GetObject<Tomato::Polygon>("poly")->GetNumberOfSides();

	if (!m_Scene->Contains("circle"))
		m_Scene->PushObject("circle", std::make_shared<Tomato::Circle>());
	else
		m_CircleSmoothness = m_Scene->GetObject<Tomato::Circle>("circle")->GetSmoothness();
...
void MainLayer::OnUpdate()
{
	...
	Tomato::Renderer::Draw(*m_Scene->GetObject<Tomato::Circle>("circle"));
